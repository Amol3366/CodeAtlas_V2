"""Generate the honest Phase 1 evaluation baseline.

Phase 1 implements one of nine query intents, so this script deliberately does
...
Usage::
    ...
    uv run python scripts/run_phase1_baseline.py --dataset tests/evaluation/cases \\
        --json-output docs/evaluation/baseline-phase-1.json \\
        --markdown-output docs/evaluation/baseline-phase-1.md [--check]
"""
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Run the Phase 1 engine over the evaluation corpus."
    )
...
def main(argv: Sequence[str] | None = None) -> int:
    try:
        ...
        if args.check:
            ...
            if not matches:
                print(
                    "Phase 1 baseline artifacts are stale. Regenerate them"
