...
| **Languages indexed** | Python, TypeScript, JavaScript, **Java**, Markdown, common config/schema formats. Go/Rust/Scala approved but not built (ADR-0065) |
...
- **Language coverage** is Python, TypeScript, JavaScript, Markdown, and common
  ...
  **Java shipped 2026-08-19 (ADR-0065); Go, Rust and Scala are approved but
  not built.** Java gets symbols, qualified names, `IMPORTS`, `CALLS`,
  `IMPLEMENTS`, and changed-symbol detection through a shared query-backed
  parser — and deliberately **no test edges and no route detection**, so
