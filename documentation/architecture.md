...
| `tree-sitter-java` / `-go` / `-rust` / `-scala` | pinned | Grammars for ADR-0065. All four are wired up through one shared query-backed engine |
...
├── docs/
│   ├── adr/                                # 0000-template + ADR-0001..0080
...
- `Snapshot` — id, repository, Git HEAD, working-tree fingerprint, lifecycle
  ...
  Current stamps: parser bundle **1.9.0**, chunker **1.1.0**, resolver **1.5.0** —
...
| Decisions and their rationale | `docs/adr/0001`–`0070` |
