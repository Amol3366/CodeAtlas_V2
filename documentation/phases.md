...
- 2026-08-04 UX/provider polish: warning codes render as plain-language notes;
  ...
  model; Settings can ask Ollama to download the typed answer model; and the
  packaged `serve --web --open` shell path was made non-cacheable and verified
  with the new Settings bundle.
...
8. **One user browser session still showed the older Settings view until manual
   reload** after the cache/navigation fix. The exact
   `uv run codeatlas serve --web --open` path was probed on 2026-08-04 and
   served the new bundle with non-cacheable shell headers, so this remains an
   environment/browser observation rather than active debug work.
