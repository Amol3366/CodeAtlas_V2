...
**This file is the maintained body of the authoritative coding-agent contract.**
The contract is exposed in the worktree as `AGENTS.md` / `CLAUDE.md` so
different coding agents can discover the entry name they expect. `CLAUDE.md`
is the Claude entry point for this same contract and forwards to this maintained
body to avoid duplicated text drifting. Citations to either name — in ADRs,
phase plans, handoff entries, and source comments — refer to the same policy
lineage. Only live pointers track the current names; historical records were
deliberately not rewritten, because a rename is not a reason to edit the
evidence a gate was approved on.
...
ADR-0007 and ADR-0008, migration `0009`, `scripts/check_phase6.ps1 -Package -Perf`, `docs/evaluation/baseline-phase-6.json`,
