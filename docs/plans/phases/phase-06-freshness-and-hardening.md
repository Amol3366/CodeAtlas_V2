...
| 7 | Performance targets from Section 19.3 still hold on the packaged build, on named hardware | `scripts/measure_phase6_perf.py` — **met 2026-07-28** (P6-08), **re-measured 2026-07-29** at Phase 4's 20-sample count: refresh p95 **1.295 s** (≤ 2 s), preflight p95 **3.103 s** (≤ 10 s), cold start 1.63 s, on the artifact over its own API. Both beat the Phase 4 source numbers, because the measurement found and fixed unbounded snapshot accumulation. It also found the server hang — recorded at the gate as an unfixed crash, since diagnosed as uvicorn's access log blocking the event loop and fixed; the 10-sample figures first recorded here (1.33 s / 3.09 s) were a consequence of that defect capping the run. See `docs/evaluation/phase-6-baseline-environment.md` |
