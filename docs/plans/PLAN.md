...
| Next gate       | none - the Section 20 development order is finished and the closeout is recorded. **New work requires an explicit user decision.** **All seven `extra_build.md` tasks are now closed** (ADR-0050 to ADR-0059), so that file had no live content and its own preamble said to delete it rather than let it rot. **Deleted 2026-08-19** on the user's instruction, after its two uniquely-recorded working rules were moved into `documentation/memory.md`; references to it in ADRs and earlier handoff entries are historical evidence and were deliberately left alone. ADR-0056 was **accepted 2026-08-17**; ADR-0057, ADR-0058 and ADR-0059 landed the same day. **ADR-0065 is DELIVERED** (`accepted` 2026-08-19): query-backed language support for **Java, Go, Rust and Scala, all four shipped**, on branch `query-backed-language-support` (not merged to `main`). `PARSER_BUNDLE_VERSION` and `RESOLVER_VERSION` are both **1.5.0**, so every pre-existing snapshot is stale and a reindex is required. **START HERE TOMORROW**, in this order: (1) **re-run `scripts/check_phase4.ps1 -SkipSync` and read the whole log.** The run that finished at session end **FAILED**: `1 failed, 2312 passed, 2 xfailed`. The cause was found and fixed -- `test_registry_resolves_python_and_ignores_unsupported_languages` used `rust` as its example of an unregistered language, and ADR-0065 registered Rust; the placeholder is now `kotlin`, whose grammar ships no `tags.scm` and therefore cannot be registered. **That fix is verified only against its own file (29 passed), never through the wrapper, so the gate is UNPROVEN and must be re-run.** **Read the log, not the exit code:** the background runner reported exit 0 while PowerShell had thrown on a failing step -- the exact trap this project already records. (2) Rule the **two declared limits** below. (3) Decide whether to merge to `main`. **The largest gap: no evaluation case measures any of the four languages.** The Deferred Register below remains the authority on what is open, and every row in it is closed or deferred with a named trigger |
...
- Transition: no phase task. Post-gate. The user asked for the bottleneck to be
  fixed after three consecutive records measured a remedy and declined it.
- New record: **ADR-0064**, status `accepted`. It **corrects ADR-0060, ADR-0061
  and ADR-0062**, all three of which attribute preflight cost to the wrong
  stage, and **closes three register rows** — the open split row, the persisted
  parse cache, and (widened, still open) the synthetic-corpus row.

**The measurement that made every earlier one wrong.** `_analyze_state`'s four
operations, timed separately on the same `GitBlobStateView` the 635 s came from:

```
list_files            1.25 s    0.4%
read_file (+hash)     0.07 s    0.0%
parse                 8.14 s    2.5%
resolve             310.24 s   97.0%
TOTAL (one side)    319.69 s
```

**Parsing the entire repository takes 8 seconds.** ADR-0060 said 99.5% of
preflight was parsing; ADR-0061 corrected that to parse-plus-resolve and left
the split unmeasured; ADR-0062 measured resolution at ~6% and called it linear.
The common error is one thing: **a timer named `parse_base` was read as timing
parsing**, when it wraps list, read, parse *and* resolve. This is the fourth
correction on this measurement, and it is the first one taken by timing the
operations separately instead of reasoning about them.

**Why ADR-0062's "linear, exponent 1.14" was honest and useless.** It was
fitted on `measure_phase4_perf.py`'s generated corpus — ~15-line Python modules,
**no Markdown**. The quadratic term lives in document references: on the real
repository `DOCUMENTS` is **117,471 of 160,687** references, and the generated
corpus has no document sections, so no mentions and no routes. The whole
quadratic term was structurally absent from the sweep.

**The defect.** `resolution.py` has claimed `O(references), not O(references x
symbols)` in its own module docstring since it was written. Three call sites
contradicted it, each scanning `symbols_by_id.values()` from inside a
per-reference loop:

| site | comparisons | what the profile showed |
| --- | ---: | --- |
| `_resolve_mention` | 112,265 x 11,502 = **1.29 billion** | `str.lower` called **880,172,549** times |
| `_RouteIndex.handlers` | 5,265 x 11,502 = 60.6 million | `name_tokens`, a regex split, called **10,529,048** times |
| `_derive_config_edges` | sections x all symbols | `_dotted_paths` called 2,226,354 times |

**The fix.** Every predicate depends only on the symbol, so all of them moved
into `_Index`: `mentionable_by_lower_name`, `handlers_by_token`,
`config_key_paths`, plus `module_suffix_to_file` and `directory_of_file` for two
smaller scans. `_RouteIndex.handlers` is memoized per route literal. Token
matching needed one new idea rather than a bucket — `tokens_match` is a
predicate, not an equality, so `routes.matching_forms` **enumerates** the closed
plural rule and turns the match into a dictionary hit.

- Files: `src/codeatlas/extraction/resolution.py`,
  `src/codeatlas/extraction/routes.py` (new `matching_forms`),
  `tests/unit/test_resolution_complexity.py` (new),
  `tests/unit/test_route_literals.py` (property test), ADR-0064, the ADR index,
  the register. **No contract, schema, migration or corpus change.**
  `SCHEMA_VERSION` stays `14`, `contract_version` stays `1.1`, and
  `RESOLVER_VERSION` stays `1.4.0` **deliberately** — it marks changes in what
  resolution concludes, and nothing about the conclusions changed.

**Verification.**

- **`scripts/check_phase4.ps1` ran to completion through the wrapper:
  `Phase 4 verification completed.`, exit code `0`, `2252 passed` in 211.71 s.**
  Every stage green in one run — frozen sync, contract schema freshness, tests,
  lint, types, dataset validation, and the Phase 0 / Phase 3 / Phase 4 baselines
  plus the ADR-0016 invariants, all `--check`. This is the authoritative gate
  evidence for this change, and it supersedes the stage-by-stage reconstruction
  below rather than contradicting it.
- **Why the reconstruction below exists, recorded rather than deleted.**
  `check_phase3.ps1` was **killed at 57% of its test run** by the environment —
  the fifth such kill this session. Nothing had failed when it died, and a
  killed run is not reported as a pass, so Phase 3's stages were run
  individually instead. Every one passed:
  ...
  - **The baselines regenerated byte-for-byte identical** — `git status` shows
    no change to any of the four files afterwards, and it showed none again
    after the full Phase 4 wrapper run. That is independent confirmation of the
    equivalence result below: if resolution's output had moved at all, the
    evaluation metrics would have moved with it. The `2252 passed` count is the
    same figure the wrapper reports, reached by two different routes.
- **Equivalence on the real repository, which is the evidence that matters
  ...
  relations**, identical `ResolutionStats` (41,524 resolved / 1,999 external /
  102,869 unresolved / 22,213 ambiguous). **313.97 s → 3.55 s, 88.4x.**
  The passing suite alone could not establish this: it passes on the *old*
...
- `tokens_match` now has **no production caller**. Kept deliberately as the
  readable statement of the rule, tied to `matching_forms` by an exhaustive
  property test, and said so in its docstring so it is not deleted as dead code.

- Next: re-measure end-to-end preflight on a clean tree, then decide whether
  resolution's remaining 3.55 s is worth a second pass.
...
- Agent: Claude Code `claude-opus-5`, branch `main`.
- Transition: no phase task. Post-gate. `extra_build.md` Task 4 **investigated,
  not implemented** — it is blocked on the design decision ADR-0034 named.
- New record: **ADR-0053**, which **corrects ADR-0051's evidence**. Ruled by the
  user 2026-08-17.
- Files: `evaluation/engine_adapter.py`, `tests/evaluation/test_engine_adapter.py`,
  baselines `-0/-3/-4`, the ADR and its index, `PLAN.md`, `extra_build.md`,
  `memory.md`. No corpus, contract or schema change.

**Task 4's premise checked out — the first time this session it did.** The three
declared edges are stored and resolved, and the lexical answers return
`relation_paths: []`. Verified by probe.

**But verifying it exposed that `CONCEPTUAL` was missing from
`SUPPORTED_INTENTS`.** `predict_exact_symbols` gates on that constant before
dispatch, so those cases were emitted as `_abstention(measured=False)` and
**never reached the engine**. **q024 had never been measured**, since it was
written — and **ADR-0051 moved q006 into the same state hours earlier**.

**This is ADR-0017 on the neighbouring constant, and it fails the opposite way.**
...
because every number it touches moves the right way.**

**The correction to ADR-0051, stated precisely.** Its conclusion **survives**:
measured through the lexical channel, q006 gives `containing_evidence_recall_at_10`
**0.9943** against the 0.9941 it reported, so it does pass containment exactly as
argued. **But the number it quoted was obtained with q006 not measured at all**,
so at the time it was reported it was denominator movement wearing a diagnosis's
clothes. A conclusion that happens to be true, verified by a measurement that
could not have shown it, is indistinguishable from a wrong one until someone
checks. I did not check `measured`.
...
**Task 4 grows to three cases.** q024 (`Sample Service DOCUMENTS service.port`)
joins q027 and q029; it could not appear on the list before because it was not
measured. A probe also found the complication that stops this being a blind
implementation: `Order flow` carries **eight** `DOCUMENTS` edges, two resolved,
and the corpus declares one — so emitting every stored path gives q027 recall
1.0 and precision 0.5, which is ADR-0038's shape exactly. **The ruling ADR-0034
asked for is still not given, and that is what blocks the work.**

**Two process failures of my own, both recorded because both are the shapes this
project keeps paying for.** `pathlib.write_text` rewrote two *source* files to
**CRLF** on Windows — the ADR-0022 hazard, on files the corpus LF guard does not
cover; caught by a git warning and a diff that was suddenly whole-file, then
normalised byte-wise. And a mutation whose anchor silently failed to apply
reported "NOT DETECTED" for a guard that was never mutated — **a mutation that
does not apply looks exactly like a test that does not catch it.** Re-run at
byte level, it is detected.

- Verification: `check_phase4.ps1 -SkipSync` **exit 0**; ruff and mypy clean;
...
- Verification, exit codes read from `$LASTEXITCODE` rather than printed output:
  `uv run pytest -q` **2240 passed, 3 skipped**, exit 0; `ruff` exit 0; `mypy --no-incremental` clean on **352** files, exit 0; `check_phase4.ps1 -SkipSync`
...
- Transition: no phase task. Post-gate. **Both decision gates carried by the
  post-closeout program since 2026-08-14 are now closed by user ruling.**
- New records: **ADR-0045** (Gate A) and **ADR-0046** (Gate B).

**Gate A — ruled "skip it, and declare the omission" (option A3).** Implemented
as WS-3.

The asymmetry ADR-0044 deliberately left: a tracked file over `max_file_bytes`
(2 MB) made `GitDiffAdapter.archive` **raise**, so one committed 3 MB CSV made a
repository impossible to preflight at all — while `DirectoryStateView` merely
skipped the same file. Now:

- `archive` skips oversized entries and returns their names beside the contents
  (`ArchiveResult`); the per-blob fallback catches the same error per file.
- **`read_blob` still raises**, on purpose: it is asked for *one specific blob*,
  and answering "here is nothing" for a file the caller named by hand is a worse
  contract than refusing. The two paths now differ deliberately and the reason
  is written at both.
- `StateView` gains `excluded_files()`; the engine unions both sides into a
  `FILE_TOO_LARGE` warning and a limitation naming the files, once rather than
  twice.

**The investigation found a second, quieter half nobody had recorded.** The
scanner has recorded `TOO_LARGE` since Phase 1, but **nothing ever carried it
into a change report** — so an oversized file was already invisible on the
*directory* side too, silently. The blob side was at least loud about it. Both
sides now declare, which means some previously-silent reports will gain a
limitation. That is the point, not a regression.

**Scope held deliberately:** only oversize is declared. An *ignored* file stays
unreported, because ADR-0044 already ruled it outside the index and accepted
that consequence; reporting it here would relitigate that ruling in warning
text.

**Gate B — ruled "a module-level answer satisfies a conceptual question", with
no ranking change** (option B1). **No code was written**, which is the correct
outcome: ADR-0030 had already investigated s001 and found no defect. Ruling the
other way would have declared a defect where an investigation found none — the
pattern corrected seven times already — spent real risk on `symbol_recall_at_10`,
which is ungated on the retrieval profile (ADR-0023), and traded an evidence hit
for a symbol hit, inverting the product's own priority.

**WS-5 is reframed rather than unblocked.** Its register row's trigger ("the
module-granularity ruling lands") is satisfied, and the ruling was *not* to
implement the coarse-chunk penalty. It is now "measure whether the bias costs
anything at the larger corpus", not "fix it".

- Contracts/migrations: **none.** `contract_version` `1.1`, `SCHEMA_VERSION`
  `14`; `PARSER_BUNDLE_VERSION`, `RESOLVER_VERSION`, `CHUNKER_VERSION`
  unchanged, so **no snapshot is stale**. No new dependency. No baseline moved.
...
- **The pinning test is inverted, not deleted.**
  `test_a_tracked_file_over_the_size_limit_fails_the_whole_comparison` existed
  so the asymmetry "cannot be altered silently in either direction"; it now
  asserts the skip, in the same place, alongside a test that `read_blob` still
...
- Verification, exit codes read from the process: `uv run pytest -q` **2236
  passed, 3 skipped** (2230 + 6 new); `ruff check src tests scripts apps` exit 0
  — it caught one now-unused import, fixed; `mypy --no-incremental` clean on
  **352** source files.
- Next: **WS-2** (subject and file path on `Finding`) and **WS-6** (lexical
  intents and relation paths) are the remaining unblocked workstreams, with
  **WS-4** (Phase 4 evidence rates, per case) the largest. Execution order is
  `documentation/extra_build.md`.
...
- Verification, exit codes read from the process: `uv run pytest -q` **2230
  passed, 3 skipped**; `ruff check src tests scripts apps` exit 0; `mypy --no-incremental src tests scripts apps` clean on **352** source files;
