...
| Generated explanations | declined | Still `declined` as a *measured* feature: `docs/evaluation/explanation-phase-7.{json,md}` records no uplift, and the A/B has not been re-run against a real provider. Shipping the providers below does not change that status (ADR-0012) |
| Concrete answer providers | available, opt-in, uplift unmeasured | `OllamaAnswerProvider` and `OpenAIAnswerProvider` ship behind a per-repository `answer_provider` setting defaulting to `none`. The governed policy required before admission is now recorded in ADR-0012; the *measurement* is not, which is why this row says available rather than admitted. Enabling one is a user opt-in, not a cleared release gate |
