...
| Generated explanations | declined | Now declined on a *measured* basis. `docs/evaluation/explanation-phase-7.{json,md}` records a real A/B against `ollama`/`llama3.2:3b` over 40 cases: all six metrics moved by exactly 0.0000, which confirms ADR-0012 decision 1 rather than praising the model — they are computed from evidence and claims, which generation never touches. The prior artifact never ran an evaluation at all (`after = before`) |
| Concrete answer providers | available, opt-in, uplift unmeasurable on this corpus | `OllamaAnswerProvider` and `OpenAIAnswerProvider` ship behind a per-repository `answer_provider` setting defaulting to `none`. The governed policy is ADR-0012 and the A/B has been run; admission still fails because no metric in this corpus *can* move under a summary-only change, and the corpus declares no ground truth for explanation quality |
| Generated prose repeats no forbidden claim | measured, weakly | Across the 40 cases declaring `forbidden_claims`, the generated summary repeated none. The check is exact-substring after casefolding — it does not stem, strip punctuation, or compare meaning, so a paraphrase passes. Evidence of not repeating a declared sentence, not of factual safety; stated as such in the artifact |
| Answer prompts carry redacted evidence only | enforced | `_redacted_prompt` passes every excerpt through `semantic.redaction.redact` before any provider — local included, because a local model can still write a secret into an answer that is pasted elsewhere. `tests/security/test_answer_generation_redaction.py` asserts the secret never reaches the provider while the evidence drawer keeps the real line |
| Answer generation cannot fabricate a citation | enforced | generated prose replaces `answer.summary` only; `answer.claims` and `evidence` pass through untouched with their original derivation. A bracketed `ev_` marker naming evidence that does not exist discards the summary and reports `GENERATED_CLAIM_INVALID` |
| Answer generation cannot fail a run | enforced | every provider fault falls back to the verified answer with a warning naming the cause (unreachable, model missing, key rejected, quota exhausted, timeout). Empty evidence produces no provider call at all, so an abstention is never dressed up in prose |
| Transmitting answer provider requires a budget | enforced | `SettingsService.update` refuses `answer_provider = openai` without a monthly token budget, checked against the resolved state so removing the budget later is refused exactly like never setting one |
| `.env` cannot enable answer generation | enforced | `build_answer_provider` reads the stored policy only; `tests/unit/test_answer_provider_factory.py` sets every answer variable and an API key, leaves the policy at `none`, and asserts `NoAnswerProvider` |
...
| `.env` cannot grant consent | enforced | every configuration variable set and the repository policy left at `none` still builds `NoEmbeddingProvider`; asserted by `tests/security/test_env_configuration.py`. Permission lives in `repository_provider_policy`, and `build_embedding_provider` has no environment override (ADR-0011) |
| Repository content cannot become configuration | enforced | `.env` is read from `$CODEATLAS_ENV_FILE` or the CodeAtlas root resolved through the package's own location — **never** the current directory. A test plants a `.env` in a repository root, runs from that directory, and asserts nothing was applied |
| Credential from `.env` absent from responses | enforced | `GET /v1/settings`, `GET /v1/models`, `/v1/repositories/{id}/diagnostics`, and the error envelope are asserted free of the key and of its last twelve characters. The loader returns applied key *names*; no value is returned, stored, or formatted |
| Malformed or hostile `.env` cannot deny service | enforced | oversized lines, binary content, unterminated quotes, and 2,000 assignments are bounded and non-fatal; a broken config line is skipped rather than raised, because deterministic operation needs no settings at all |
| Env files excluded from scans | enforced | `.env`, `.env.*`, and `*.env` are default-ignored with `!.env.example` re-included. Blueprint §8.11 conformance and hygiene — **not** a leak fix: a `.env` classifies as `unknown` with no parser, so its contents were never parsed, chunked, written to FTS, or embedded; only its path was searchable |
