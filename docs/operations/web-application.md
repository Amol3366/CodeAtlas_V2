...
- **Settings** — `/settings`, reached from the sidebar header. Chooses the
  ...
  coverage panels, fresh settings/model refetch on mount, and — for the local
  embedding provider — a free-text **Embedding model** field gated behind a
  **Check model** step that reports the model's measured vector width
  (ADR-0014). With no repository
